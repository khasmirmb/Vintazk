<!-- Main modal -->
<div id="modal-{{ Str::slug($team->name, '-') }}"
     tabindex="-1"
     aria-hidden="true"
        ...
        <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
            ...
            <div class="flex items-center justify-between p-4 md:p-5 border-b border-gray-200 dark:border-gray-600 rounded-t">
                ...
                <button type="button"
                        ...
                        data-modal-hide="modal-{{ Str::slug($team->name, '-') }}">
                ...
                <div class="items-center justify-between p-4 bg-white border border-gray-200 rounded-lg shadow-sm sm:flex dark:border-gray-700 sm:p-6 dark:bg-gray-800 h-36">
                    <div class="w-full">
                        ...
                        <span class="text-2xl font-bold leading-none text-gray-900 sm:text-3xl dark:text-white">
                            {{ number_format($team->avg_performance_rating ?? 0, 2) }}%
                        </span>
                    </div>
                    <div class="w-full" id="team_overall_chart{{ Str::slug($team->name, '-') }}"></div>
                </div>
            </div>
        </div>
    </div>
...
@php
    $data = $monthlyPerformance[$team->name] ?? collect();
    $months = $data->pluck('month');
    $ratings = $data->pluck('performance_rating');
@endphp


<script type="module">
if (document.getElementById('team_overall_chart{{ Str::slug($team->name, '-') }}')) {
    const options = {
        series: [{
            name: 'Performance',
            data: @json($ratings),
    ...
    const chart = new ApexCharts(
        document.getElementById('team_overall_chart{{ Str::slug($team->name, '-') }}'),
        options
    );
    chart.render();
}
